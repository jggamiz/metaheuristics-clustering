...
La Búsqueda Local parte de una solución inicial aleatoria y explora su entorno buscando mejoras iterativas en el valor de fitness.

El entorno de una solución está formado por todas aquellas soluciones que se obtienen moviendo una instancia de su cluster actual a otro distinto.
...
\begin{verbatim}
	...
	solution ? generar solucion inicial aleatoria
	...
	improvement ? false
	generar lista de movimientos (instancia, cluster)
	barajar lista
	
	para cada movimiento m hacer
	
	neighbor ? aplicar movimiento(solution, m)
	neighbor_fitness ? fitness(neighbor)
	
	evaluations++
	
	si neighbor_fitness < fitness entonces
	solution ? neighbor
...
El operador de generación de vecinos consiste en seleccionar una instancia y moverla a un cluster distinto.

\begin{verbatim}
	generarVecino(solution, instancia i, cluster c)
	
	neighbor ? copia de solution
	neighbor[i] ? c
	
	devolver neighbor
\end{verbatim}

No se permite aplicar el movimiento si al retirar la instancia de su cluster original éste quedaría vacío.

\subsubsection{Exploración del entorno}

El entorno se explora generando todas las posibles combinaciones de instancia y cluster distinto del actual. Estas combinaciones se barajan aleatoriamente para evitar sesgos en el orden de exploración.

La estrategia utilizada es \textit{First Improvement}: en cuanto se encuentra un vecino con mejor fitness que la solución actual, se acepta inmediatamente y se vuelve a generar el entorno desde la nueva solución.

\begin{verbatim}
	explorarEntorno(solution)
...
La solución inicial se genera aleatoriamente asegurando que todos los clusters contengan al menos una instancia.

\begin{verbatim}
	generarSolucionInicial()
	
	barajar lista de instancias
	
	asignar una instancia distinta a cada cluster
	
	para el resto de instancias
	asignar cluster aleatorio
	
	devolver solucion
\end{verbatim}

Este procedimiento evita la aparición de clusters vacíos desde el inicio de la búsqueda, lo que facilita la exploración del espacio de soluciones.
