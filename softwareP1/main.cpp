...
void print_result(ResultMH<int>& result, long int seed, float deviation, int infeasibility, double time_sec) {
    ...
    cout << "Time (seconds): " << time_sec << endl;
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 4) {
        cout << "Running 50 iterations with different seeds..." << endl;
        for (int i = 0; i < 50; i++) {
    ...
    // Preparar archivos CSV en modo "append" (añadir al final)
    ...
    for (size_t i=0; i<algoritmos.size(); i++) {
        ...
        cout << "Time (seg):    " << sum_time / seeds.size() << endl;

        // Escribir fila media en CSV de medias (solo si se corrieron las 50 semillas)
        if (seeds.size() > 1) {
