...
ResultMH<int> GreedySearch::optimize(Problem<int>& problem, int maxevals) {
    ...
    // 1. Averiguar el dominio de cada dimensión (máximos y mínimos)
    ...
    // 2. Generar k centroides iniciales aleatorios dentro del dominio
    ...
    while (changed) {
        ...
        for (int i : indices) {
            ...
            for (int c=0; c<k_clusters; ++c) {
                // a) Distancia Euclidea de la instancia al centroide 'c'
                double dist = 0.0;
                for (int f=0; f<num_features; ++f) {
                ...
                // b) Inviabilidades si asignamos 'i' al cluster 'c'
                int violations = 0;
                ...
                // c) Criterio de selección: menor inviabilidad y desempate por distancia
    ...
    // Una vez ha convergido, evaluamos la solución final usando el marco MH
