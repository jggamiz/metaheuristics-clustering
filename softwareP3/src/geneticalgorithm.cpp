...
tSolution<int> GeneticAlgorithm::run() {
    ...
    while (current_evals < max_evals) {
        ...
        // Hibridación Memética cada 10 generaciones (Exclusivo AM-BEST)
        if (generations % 10 == 0) {
            applyMemetic();
        }
    }
...
// Selección (Esquema Generacional Puro)
...
// Reemplazo (Esquema Generacional con Elitismo)
void GeneticAlgorithm::replacement() {
    ...
    // Si el mejor hijo es peor que el mejor padre, conservamos al mejor padre (Elitismo)
...
void GeneticAlgorithm::applyMemetic() {
    // Calculamos el 10% de los mejores
    ...
    // Aplicar Búsqueda Local Suave solo a ese top 10%
