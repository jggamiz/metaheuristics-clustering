...
tSolution<int> ProblemPAR::createSolution() {
    ...
    // Force one element per cluster
    for (int c = 0; c < num_clusters; ++c)
        sol[available[c]] = c;
 
    // Assign the rest uniformly at random
    for (int i = num_clusters; i < n; ++i)
        sol[available[i]] = Random::get(0, num_clusters - 1);
...
bool ProblemPAR::isValid(const tSolution<int>& solution) {
    vector<bool> has_element(num_clusters, false);
    ...
    // Every cluster must have at least one element
    for (int c = 0; c < num_clusters; ++c)
        if (!has_element[c]) return false;
