...
public:
    ...
    // Read-only accessors (used by GRASP construction)
    const vector<vector<double>>& getFeatures() const { return features; }
    const vector<vector<int>>& getMLConstraints() const { return ml_constraints; }
    const vector<vector<int>>& getCLConstraints() const { return cl_constraints; }
};
