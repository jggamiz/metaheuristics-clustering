...
- `inc`: es un directorio incluido en la plantilla. Lo hemos modificado para incluir las cabeceras de nuestros algoritmos y problema:
    - `geneticalgorithm.h`: contiene la declaración de la clase `GeneticAlgorithm`, que implementa de forma autónoma Algoritmos Genéticos Generacionales (AGG), Estacionarios (AGE) y Algoritmos Meméticos (AM) basados en un AGG. Permite configurar tipo de evolución, operador de cruce (uniforme o segmento fijo), tamaño de población, probabilidades de cruce y mutación, así como la frecuencia de la búsqueda local (para las variantes meméticas).
...
- `src`: es un directorio incluido en la plantilla. Lo hemos modificado para incluir las implementaciones de las clases definidas en `inc`:
    - `geneticalgorithm.cpp`: contiene la implementación completa de los métodos de `GeneticAlgorithm`.
